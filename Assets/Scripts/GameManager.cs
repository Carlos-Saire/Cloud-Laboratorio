...
public class GameManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < cardPool.Length; i++)
        {
            for (int l = 0; l < 2; l++)
            {
                ...
                go.GetComponent<Card>().Initialize(cardPool[i].index,
                cardPool[i].sprite, backcardSprite);
    ...
    public void PickACard()
    {
        ...
        {
            index++;
            UnityEngine.EventSystems.EventSystem.current.currentSelectedGameObject.GetComponent
            <Card>().Flip();
            if (!firstGuess)
            {
                firstGuess = true;
    ...
    private IEnumerator EvaluateCards()
    {
        ...
        if (firstchoise.Index() == secondchoise.Index() &&
        firstchoise.GetInstanceID() != secondchoise.GetInstanceID())
        {
